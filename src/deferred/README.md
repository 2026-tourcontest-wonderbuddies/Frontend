...
저장함(장소·코스)은 이후 서버로 옮겨졌다
(`/api/places/saved/`, `/api/courses/saved/` — `src/hooks/useSavedPlaces.ts`, `src/hooks/useSavedCourses.ts`).
장소 검색은 엔드포인트가 생기면 `src/data/places.ts`의 `filterPlaces()`만 API 호출로
갈아끼우면 되고, 화면은 그대로 둔다.
